...
// visitor's staging queue. The client then opens the review/queue modal; only
// an explicit "Save to portfolio" click commits (entitled → promote → build
// bar; everyone else → plan → Stripe checkout → webhook promotion). This route
// NEVER promotes — it only stages.
//
// It always stages the FULL demo list (idempotent upsert on
// (clerk_id, canonical_address)), even when the visitor already owns some of
// the buildings — so the review queue always shows and re-claiming never
// silently redirects. Re-staging an already-owned building is harmless: the
// portfolio row is untouched until the user Saves, and re-promotion is
// idempotent.
//
...
// parcel characteristics the seed pipeline derived.
...
export async function POST(request: Request) {
  ...
  // Always stage the full demo list (idempotent) so the review queue opens
  // every time — even on a re-claim where the visitor already owns some rows.
  // Nothing is committed here, so re-staging an owned building is harmless.

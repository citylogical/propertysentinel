...
export async function POST(request: Request) {
  ...
  // Open, already-enriched complaints in the last 12 months — WOLI refresh only.
  // backfill/start handles unenriched / retro-paraphrase / missing work_order_steps.
  ...
  if (queue.length === 0) {
    return NextResponse.json({
      ...
      failed: 0,
      status: 'done',
      message: 'No open enriched complaints found to refresh.',
    })
  }
